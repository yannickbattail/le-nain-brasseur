...
export class Gui {
  ...
  loose() {
    if (
      this.engine.status == BrewerDwarfStatus.LOOSE &&
      this.engineStatus != BrewerDwarfStatus.LOOSE
    ) {
      this.endGame(
        false,
        "Tu as trop vomis c'est pas bien!! Tu aura plus de chance le(a) prochain(e) foie(s).",
      );
      this.engineStatus = this.engine.status;
    }
    if (
      this.engine.status == BrewerDwarfStatus.WIN &&
      this.engineStatus != BrewerDwarfStatus.WIN
    ) {
      this.endGame(
        true,
        "C'est bien, tu as gagné ! Mais guette les prochaines évolutions du jeu.",
      );
      this.engineStatus = this.engine.status;
    }
  }
  ...
  private listRecipeReferences(): string {
    return this.engine.recipes
      .map((res) => this.listRecipeReference(res))
      .join("");
  }
  ...
  private listRecipeReference(recipe: RecipeReference): string {
    ...
    h += recipe.getName();
    h +=
      "<button onclick=\"engine.prepareBrew('" +
      recipe.getName() +
      "')\">Préparer</button>";
    h += "</div>";
    return h;
  ...
  private displayPlayerRecipe(recipe: Recipe): string {
    ...
    h +=
      "<tr><th>" +
      Gui.htmlEntities(recipe.getName()) +
      " (" +
      this.displayScore(recipe.score) +
      ")</th>";
    h +=
      '<th colspan="2">À partir de: ' +
      recipe.recipeRef?.getName() +
      "</th></tr>";

    recipe
      .getCookingSteps()
      .forEach((res) => (h += this.displayCookingStep(res)));
    h +=
      '<tr><td colspan="3"><button onclick="engine.reprepareBrew(\'' +
      recipe.getName() +
      "')\">Préparer</button></td></tr>";
    h += "</table></div>";
    return h;
  }
      ...
      if (paramIndex < params.length) {
        ...
        if (param.name == "durée") {
          ...
          h +=
            ...
            param.name +
            '">' +
            this.displayQuantity(Q(param.value, param.resource)) +
            "</div></td>";
        } else {
          h +=
            '<td><div title="' +
  ...
  private editRecipe(recipe: Recipe): string {
    let h = '<div style="display: inline-block;"><table border="1">';
    h +=
      '<tr><th colspan="3">Brassée partir de: ' +
      recipe.recipeRef?.getName() +
      "</th></tr>";
    h += "<tr>";
    h += "<td>Nom: </td>";
    h +=
      '<td colspan="2"><input type="text" id="recipeName" value="' +
      recipe.getName() +
      '" onchange="engine.checkBrew()" /></td>';
    h += "<td><b>Note:</b> " + this.displayScore(recipe.score) + "</td>";
    h +=
      '<td><span class="problem"><b>Problème:</b> ' +
      (recipe.problem != null ? recipe.problem : "") +
      "</span>";
    ...
    recipe
      .getCookingSteps()
      .forEach((step, i) => (h += this.editCookingStep(i, step)));
    h += "<tr>";
    const disabled =
      recipe.hasProblem() || recipe.analysisLevel == AnalysisLevel.NONE
        ...
        : "";
    h +=
      '<td colspan="3"><button onclick="engine.brew()" ' +
      disabled +
      ">Brasser!</button>";
    h += "<td>&nbsp;</td>";
    const storageRes = this.engine.player.getResourceInStorage(
      ADVISE_COST.getResource().getName(),
    );
    let cssClass = "notAvailableResource";
    if (
      storageRes != null &&
      storageRes.getQuantity() >= ADVISE_COST.getQuantity()
    ) {
      cssClass = "availableResource";
    }
    h +=
      '<td><button onclick="engine.advise()">Conseils ' +
      this.displayQuantity(ADVISE_COST, cssClass) +
      "</button></td>";
    h += "</tr>";
    h += "</table></div>";
    return h;
  ...
  private displaySellBrew(recipe: Recipe, player: IPlayer): string {
    ...
    let sell = "Acheter";
    if (
      recipe.getArticle().resource.getResource().getName() == GOLD.getName()
    ) {
      sell = "Vendre";
    }
    return (
  ...
  private updateGui() {
    ...
    NodeUpdate.updateDiv(
      "storageIngredient",
      this.displayStorageCategory("Ingrédients", "Ingredient"),
    );
    NodeUpdate.updateDiv(
      "storageItem",
      this.displayStorageCategory("Items", "Item"),
    );
