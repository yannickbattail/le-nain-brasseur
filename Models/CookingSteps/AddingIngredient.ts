...
export class AddingIngredient extends CookingStep {
    ...
    public static load(obj: unknown): AddingIngredient {
        const data: AddingIngredient = obj as AddingIngredient;
        const stepParameters = (data.stepParameters as Array<unknown>).map((p) => StepParameter.load(p));
