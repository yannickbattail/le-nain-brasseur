...
import { CategorizedItem } from './Resources/CategorizedItem.js';
import { CategorizedMaterial } from './Resources/CategorizedMaterial.js';
...
export class Player implements IPlayer {
    ...
    public static load(data: unknown): Player {
        ...
        player.preventNegativeStorage = obj.preventNegativeStorage;
        player.storage = obj.storage.map((p) => ClassLoader.load(p) as Quantity);
        player.recipes = obj.recipes.map((p) => ClassLoader.load(p) as Recipe);
    ...
    public getRecipeNameByName(recipeName: string): Recipe | null {
        const recipes: Recipe[] = this.recipes.filter((src) => src.getName() == recipeName);
        if (recipes.length == 0) {
            return null;
        }
        return recipes[0];
